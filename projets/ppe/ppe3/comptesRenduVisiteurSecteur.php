...
try {
    $bdd = new PDO('mysql:host=localhost;dbname=alexismopgppegsb;charset=utf8', 'root', '');
} catch (Exception $e) {
...
$req = $bdd->prepare('SELECT DISTINCT(secteur) FROM visiteur_ppe3 ORDER BY secteur ASC');
$req->execute();
...
<h1 class="center"> Liste des comptes-rendus des visiteurs par secteur </h1>
...
<label for="cr_visiteur">Type</label><br />
                <select name="cr_visiteur" id="cr_visiteur">
                    ...
                    <?php
for ($i = 0; $i < $test; $i++) {
    echo '<option value="' . $donnees[$i]['secteur'] . '">' . $donnees[$i]['secteur'] . '</option>';
}
?>
                </select>
        </div>
...
if (isset($_POST['cr_visiteur'])) {
    ...
    $requete2 = $bdd->prepare('SELECT * FROM rapport_visite WHERE vis_matricule IN (
    SELECT vis_matricule FROM visiteur_ppe3 WHERE secteur = :secteur) ');
    $requete2->execute(array('secteur' => $_POST['cr_visiteur']));
